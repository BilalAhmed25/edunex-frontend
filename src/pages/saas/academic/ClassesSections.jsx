...
const ClassesSections = () => {
    ...
    const handleAddSection = async (e) => {
        ...
        try {
            ...
            fetchData(); // Refresh main list to show new section count/names
        } catch (err) {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white p-4 rounded-xl border flex justify-between items-center">
                <div>
                    <h4 className="card-title text-2xl font-bold poppins text-dark dark:text-white">Manage Classes</h4>
                    <p className="text-sm text-muted poppins mt-1">Configure academic levels and their sections.</p>
                </div>
                <Button
                    text="Add New Class"
                    icon="heroicons-outline:plus"
                    className="btn-primary poppins px-6 py-2"
                    onClick={() => { resetForm(); setIsOpen(true); }}
                />
            </div>

            {loading ? (
                <GridSkeleton count={classes.length || 6} />
            ) : classes.length === 0 ? (
                <div className="col-span-full text-center bg-white dark:bg-slate-800 p-20 rounded-2xl border dark:border-slate-700">
                    ...
                    <Button text="Get Started" className="btn-primary btn-sm" onClick={() => setIsOpen(true)} />
                </div>
            ) : (
                    ...
                    {classes.map((cls) => {
                        ...
                        return (
                            <div key={cls.ID} className="group transition-all duration-300 transform hover:-translate-y-1">
                                <div className="card h-100 bg-white dark:bg-slate-800 border-none rounded-2xl p-6 relative overflow-hidden flex flex-column shadow-none border">
                                    <div className="absolute top-0 right-0 p-3 flex gap-2">
                                        <button onClick={() => handleEditClass(cls)} title="Edit Class" className="p-2 bg-slate-50 dark:bg-slate-700 text-slate-500 hover:text-primary-500 rounded-lg transition-colors">
                                            <Icon icon="heroicons-outline:pencil-alt" />
                                        </button>
                                        <button onClick={() => handleDeleteClass(cls.ID)} title="Delete Class" className="p-2 bg-slate-50 dark:bg-slate-700 text-slate-500 hover:text-danger-500 rounded-lg transition-colors">
                                    ...
                                    <div className="flex-grow space-y-5">
                                        <div>
                                            <span className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Available Faculty</span>
                                            <div className="flex flex-wrap gap-1.5">
                                                {(cls.Faculties || "General").split(", ").filter(Boolean).map((f, i) => (
                                                    <Badge key={i} label={f} className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-semibold text-[10px] px-2" />
                                                ))}
                                            </div>
                                        </div>

                                        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3  relative">
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="text-xs font-bold text-muted uppercase tracking-wider block">Sections</span>
                                                <button
                                            ...
                                            <div className="flex flex-wrap gap-1.5 min-h-[30px]">
                                                {parsedSections.length > 0 ? parsedSections.map((s) => (
                                                    <div
                                                        key={s.ID}
                                                        onClick={() => handleOpenSections(cls)}
                                                        className="px-2 py-1 bg-white dark:bg-slate-800 rounded-md text-xs font-medium text-slate-600 dark:text-slate-300 flex items-center gap-1.5 cursor-pointer hover:border-primary-400 transition-colors"
                                                    >
                                                        {s.Name}
                                                        <Icon icon="heroicons:pencil-square" className="w-3 h-3 text-slate-400 group-hover:text-primary-400" />
                                                    </div>
                                                )) : (
                                                    <span className="text-xs text-muted italic poppins opacity-60">No sections added</span>
                                                )}
                                            </div>
                                        </div>
                                    </div>
